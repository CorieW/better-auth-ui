...
The `@daveyplate/better-auth-ui` library provides b@coriewount settings views accessible through the `account` configuration. By default, these views are located at `/account/settings`, `/account/security`, `/account/api-keys`, and `/account/organizations`.
...
- Auth views remain at: `/auth/@coriew/auth/sign-up`, etc.
