...
This guide covers integrating `@coriew/better-auth-ui` into your Next.js project.
...
Follow these steps to set up `@coriew/better-auth-ui` in your [Next.js](https://nextjs.org) project using the **App Router**:
...
import { AuthUIProvider } from "@coriew/better-auth-ui"
...
import { AuthView } from "@dave@coriewer-auth-ui"
...
export function generateStaticParams() {
    return Object.values(accou@coriew).map((path) => ({ path }))
}
...
@coriew
1. Update your `AuthUIProvider`@corieweParams` to get the current slug from the URL
